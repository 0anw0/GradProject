...
const getBubbleKeys = async (communityKey, roomKey, uuid,setBubbleState) => {

    let hiddenBubbles = []
    firebase.database()
        .ref(`authenticatedUsers/${uuid}/hiddenBubbles/`)
        .on('value' , snap => {
            if (snap != null) {
                snap.forEach(child => {
                    hiddenBubbles.push(child.val().bubbleKey)
                })
            }
        })
                            ...
                            if (element != null) {
                                firebase.database().ref(`authenticatedUsers/${element.val().uuid}`)
                                    .once('value', user => {
                                        ...
                                        if (user != null) {
                                            //console.log('hidden:-', !hiddenBubbles.includes(child.val().bubbleKey))
                                            if (!hiddenBubbles.includes(child.val().bubbleKey)) {
                                                bubbles.push({
