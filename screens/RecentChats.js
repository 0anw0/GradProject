...
export default class App extends React.Component {
  ...
  async signInToDB(email, password) {
  ...
  ShowChat = (id) => {
    ...
    if (this.state.hiddenList.length == 0)
      this.setState({ hiddenChats: false });
  ...
  getChats(list) {
    ...
    list.forEach((element) => {
      ...
      let hidden,
        id = "";
      db.ref("/" + element.room_route + "/hidden").on("value", (snap) => {
        hidden = snap.val();
      });
      db.ref("/" + element.room_route + "/id").on("value", (snap) => {
        id = snap.val();
      });
        ...
        snap.forEach((child) => {
          if (!hidden) {
            if (timestamp <= child.val().timestamp) {
              ...
              if (flag1) chats.pop();
              chats.push({
                // pushing choosing message to the array.
                key: child.key,
  ...
  render() {
    return (
      <View style={{ marginTop: StatusBar.currentHeight, flex: 1 }}>
        ...
        >
          <View
            style={{ backgroundColor: "#d6f1ff", borderRadius: 15, margin: 5 }}
          >
            <FlatList
          ...
          <View>
            <TouchableOpacity onPress={this.HiddenListOpt}>
              {this.state.hiddenChats ? (
                <Text
                  style={{ paddingLeft: 10, fontWeight: "bold", color: "grey" }}
                >
                  {" "}
                  Hidden({this.state.hiddenList.length}){" "}
                </Text>
              ) : (
                <Text></Text>
              )}
